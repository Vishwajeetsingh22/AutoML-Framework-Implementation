...
🎯 Objective
Automatically train multiple ML models on a given dataset and select the best one, without writing dataset-specific model-selection code each time.
...
src/data_preprocessing.py is fully generic — a new dataset can be plugged in just by adding an entry to DATASET_CONFIGS (path, target column, columns to drop, and any columns where 0 really means "missing"). For every dataset it will:

Drop irrelevant identifier columns (e.g. Loan_ID)
Replace implausible zero values with the column median (Diabetes)
Impute missing values — median for numeric columns, mode for categorical columns (Loan has real missing values in Gender, Self_Employed, Credit_History, etc.)
One-hot encode categorical predictors
Label-encode non-numeric targets
...
This preprocesses all three datasets, trains all 5 models on each, saves accuracy-comparison charts to screenshots/, saves the winning model for each dataset to outputs/, and writes outputs/automl_report.txt + outputs/automl_report.json.
...
Best Model = Random Forest
Accuracy = 83.8%
Full per-model accuracy breakdowns are in outputs/automl_report.txt and outputs/automl_report.json. See documentation/project_report.pdf for the complete write-up.
...
All model files in outputs/ are already generated — re-running generate_report.py will regenerate them deterministically (fixed random_state=42 on every model and split).
To plug in a new dataset, just add a config entry to DATASET_CONFIGS in src/data_preprocessing.py.
